...
int arm_step(arm_core p) {
  ...
  // Print the values of registers R0, R1, and R2
  uint32_t *addrR0 = registers_get_address(p, 0);
  uint32_t valR0;
  arm_read_word(p, *addrR0, &valR0);
  printf("valR0 = %u\n", valR0);

  uint32_t *addrR1 = registers_get_address(p, 1);
  printf("addrR1 = %hx\n", *addrR1);
  uint32_t valR1;
  arm_read_word(p, *addrR1, &valR1);
  printf("valR1 = %u\n", valR1);

  uint32_t *addrR2 = registers_get_address(p, 2);
  uint32_t valR2;
  arm_read_word(p, *addrR2, &valR2);
